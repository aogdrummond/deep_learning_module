...
class SFUN(object):
    ...
    def setup_model(self):
        ...
        self.checkpoints_path = os.path.join(
            self._config["training"]["session_dir"], "checkpoints"
        )
        ...
        config_path = os.path.join(
            self._config["training"]["session_dir"], "config.json"
        )
        ...
        if os.path.exists(self.checkpoints_path) and util.dir_contains_files(
            ...
            checkpoints.sort(
                key=lambda x: os.stat(os.path.join(self.checkpoints_path, x)).st_mtime
            )
    ...
    def build_model(self, train_bn=True):
        ...
        inputs_sf = Input(
            (
                self._config["dataset"]["xSamples"],
                self._config["dataset"]["ySamples"],
                self.num_freq,
            ),
            name="inputs_sf",
        )
        ...
        def encoder_layer(sf_in, mask_in, filters, kernel_size, bn=True):
            conv, mask = util.PConv2D(
                filters,
                kernel_size,
                strides=2,
                padding="same",
                name="encoder_partialconv_" + str(encoder_layer.counter),
            )([sf_in, mask_in])
            if bn:
                conv = BatchNormalization(
                    name="encoder_bn_" + str(encoder_layer.counter)
        ...
        def decoder_layer(
            sf_in, mask_in, e_conv, e_mask, filters, kernel_size, bn=True
        ):
            up_sf = UpSampling2D(
                size=(2, 2), name="upsampling_sf_" + str(decoder_layer.counter)
            )(sf_in)
            up_mask = UpSampling2D(
                size=(2, 2), name="upsampling_mk_" + str(decoder_layer.counter)
            )(mask_in)
            ...
            conv, mask = util.PConv2D(
                ...
                kernel_size,
                padding="same",
                name="decoder_partialconv_" + str(decoder_layer.counter),
            )([concat_sf, concat_mask])
            if bn:
                conv = BatchNormalization(
                    name="encoder_bn_" + str(decoder_layer.counter)
                )(conv)
        ...
        d_conv8, d_mask8 = decoder_layer(
            d_conv7, d_mask7, inputs_sf, inputs_mask, self.num_freq, 3
        )
        outputs = Conv2D(self.num_freq, 1, activation="sigmoid", name="outputs_sf")(
            d_conv8
        )
    ...
    def loss_total(self, mask):
        ...
        def loss(y_true, y_pred):
            ...
            return (
                self._config["training"]["loss"]["valid_weight"] * valid_loss
                + self._config["training"]["loss"]["hole_weight"] * hole_loss
            )
    ...
    def fit_model(
        self, train_generator, num_steps_train, val_generator, num_steps_val, epochs
    ):
        """Fit SFUN
        ...
        self.model.fit_generator(
            ...
            callbacks=[
                CSVLogger(
                    "".join(
                        [
                            self._config["training"]["session_dir"],
                            "/",
                            self.history_filename,
                        ]
                    ),
