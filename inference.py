...
def get_test_filenames(test_path):
    ...
    for subfolder in os.listdir(test_path):
      ...
      filenames_list = [os.path.join(subfolder,filename) for filename in os.listdir(subfolder_path) if filename.endswith('.mat')]  
...
def simulated_data_evaluation(config_path):
    ...
    test_path = "".join([config['dataset']['path'],'/', 'simulated_soundfields','/', 'test'])
...
def visualize_multiple_mics(config_path):
    ...
    for num_mics in searcher_config["visualization"]["num_mics"]:
      ...
      mask_generator = data.MaskGenerator(config['dataset']['xSamples']//config['dataset']['factor'], config['dataset']['ySamples']//config['dataset']['factor'], len(frequencies), num_mics=num_mics)
