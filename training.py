...
def model_finetuning(config_path:str):
    ...
    for hp, epochs in zip(searcher_config["training"][evaluated_field],searcher_config["training"]["num_epochs"]):
      ...
      # Create session directory
      if 'session_dir' not in config['training'] or os.path.exists(config['training']['session_dir']):
    ...
    #STEP 2
    for hp1,hp2,epochs in zip(searcher_config["training"]["num_steps_train"],searcher_config["training"]["num_steps_val"],searcher_config["training"]["num_epochs"]):
...
def best_model_path(id_sessions_trained:list,root_path:str)->str:
  ...
  sessions_pathes = ["".join([root_path,"session_",str(session_id)]) for session_id,root_path in zip(id_sessions_trained,[root_path]*len(id_sessions_trained))]
